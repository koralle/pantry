...
export const Default = meta.story({
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getAllByRole("heading", { level: 1, hidden: true })
    ).toHaveLength(1);
    const passkeyButton = await canvas.findByRole("button", {
      name: "パスキーでログイン",
    });
    const email = canvas.getByLabelText("メール");
    expect(
      passkeyButton.compareDocumentPosition(email) &
        Node.DOCUMENT_POSITION_FOLLOWING
    ).not.toBe(0);
    await expect(canvas.getByText("または")).toBeInTheDocument();
    await expect(canvas.getByLabelText("パスワード")).toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: "サインイン" })
    ).toBeEnabled();
  },
});
...
export const InvalidCredentials = meta.story({
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(
        canvas.getAllByText("メールまたはパスワードを確認してください")
      ).toHaveLength(2);
    });
  },
});
...
export const Pending = meta.story({
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      await expect(
        canvas.getByRole("button", { name: "サインイン中..." })
      ).toBeDisabled();
    });
    await expect(
      canvas.getByRole("group", { name: "サインイン" })
    ).toBeDisabled();
  },
});
...
export const WebAuthnUnavailable = meta.story({
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(
        canvas.queryByRole("button", { name: "パスキーでログイン" })
...
export const PasskeyCancelled = meta.story({
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      await expect(
        canvas.getByRole("button", { name: "パスキーでログイン" })
      ).toBeEnabled();
    });
    ...
    mocked(authClient.signIn.passkey).mockImplementation(
      async (opts?: { autoFill?: boolean }) => {
        if (opts?.autoFill === true) {
          return await neverSignIn();
        }
        return cancelledPasskeyResult;
      }
    );
  },
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByLabelText("メール")).toHaveValue(
      "user@example.com"
    );
    await expect(
      canvas.getByRole("button", { name: "サインイン" })
