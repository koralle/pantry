...
async function loadEditorForStory(
  id: string
...
const meta = preview.meta({
      ...
      router: {
        ...
        routeOverrides: {
          ...
          "/_protected/bookmarks/$id/edit": {
            loader: async (context: { params: { id: string } }) =>
              await loadEditorForStory(context.params.id),
          },
        },
      },
    },
  ...
  beforeEach: async () => {
    ...
    updateBookmarkDep = async () => ({
      kind: "updated",
      id: v.parse(bookmarkIdSchema, bookmarkId),
    });
    installRpcFetchStub();
...
export const InitialLoading = meta.story({
  beforeEach: async () => {
    findBookmarkEditorDep = async () => await new Promise<never>(() => {});
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
