...
function passkey(
  overrides: Partial<ManagedPasskey> & { readonly id: string }
): ManagedPasskey {
  return {
...
export const NamedAndFallbackNames = meta.story({
  beforeEach: async () => {
    mocked(authClient.passkey.listUserPasskeys).mockResolvedValue({
      data: [
        passkey({ aaguid: googleAaguid, id: "pk-named", name: "仕事用キー" }),
        passkey({ aaguid: googleAaguid, id: "pk-google", name: null }),
        passkey({ aaguid: unknownAaguid, id: "pk-unknown", name: null }),
      ],
      error: null,
    });
  },
...
export const AddWithoutPrefillingName = meta.story({
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.queryByLabelText("表示名")).not.toBeInTheDocument();
    expect(
      mocked(authClient.passkey.addPasskey).mock.calls[0]?.length ?? 0
    ).toBe(0);
  },
});
...
export const RenameUpdatesList = meta.story({
  beforeEach: async () => {
    ...
    mocked(authClient.passkey.updatePasskey).mockImplementation(
