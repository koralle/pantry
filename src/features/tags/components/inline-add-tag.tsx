...
export const InlineAddTag = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setName("");
      await navigate({ search: { limit: 50, offset: 0 }, to: "/tags" });
    } catch (error) {
  ...
  return (
    <form className={inlineAddTag} onSubmit={handleSubmit}>
      <div className={field}>
        ...
        <div className={fieldUrlRow}>
          ...
          />
          <StyledButton type="submit" isDisabled={mutation.isPending}>
            <Plus size={16} aria-hidden />{" "}
            {mutation.isPending ? "追加中..." : "追加"}
          </StyledButton>
        </div>
      </div>
      {tagError === null || tagError === undefined ? null : (
