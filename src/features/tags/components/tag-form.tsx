...
export const TagForm = ({
  ...
  return (
    <form className={workbenchForm} {...getFormProps(form)} action={submit}>
      {formError === null || formError === undefined ? null : (
        <div className={formSummary} role="alert" aria-live="polite">
          <p>
            <CircleAlert size={16} aria-hidden /> {formError}
          </p>
        </div>
      )}
      ...
      <fieldset className={workbenchFields} disabled={isPending}>
        ...
        <div className={field}>
          ...
          />
          {fields.name.errors ? (
            <p className={fieldError}>{fields.name.errors[0]}</p>
          ) : null}
        </div>
