...
function signalOf(init: RequestInit | undefined): AbortSignal | null | undefined {
  if (!init) {
    return
  }
...
describe('fetchPageTitle', () => {
  describe('hostname resolution', () => {
    ...
    test('rejects a hostname that resolves to a private address', async () => {
      ...
      await expect(fetchPageTitle('https://127.0.0.1.nip.io', resolveHostname)).resolves.toBe(
        unavailableTitle
      )
    ...
    test.each(['fec0::1', '64:ff9b:1::1'])(
      ...
      async (address) => {
        ...
        await expect(fetchPageTitle('https://example.com', resolveHostname)).resolves.toBe(
          unavailableTitle
        )
        expect(fetch).not.toHaveBeenCalled()
      }
      ...
      async (address, queryType, recordType) => {
        const fetch = vi.fn((input: RequestInfo | URL) => {
          ...
          if (url.hostname === 'cloudflare-dns.com') {
            ...
            if (url.searchParams.get('type') === queryType) {
              answers.push({ data: address, type: recordType })
            }
        ...
        await expect(fetchPageTitle('https://example.com')).resolves.toBe(unavailableTitle)
      ...
      const fetch = vi.fn((input: RequestInfo | URL) => {
        ...
        if (url.hostname === 'cloudflare-dns.com') {
          ...
          if (url.searchParams.get('type') === 'A') {
            answers.push({ data: '93.184.216.34', type: 1 })
          }
  ...
  describe('timeouts', () => {
    ...
    test('returns null when a DoH lookup aborts at the timeout', async () => {
      const controller = new AbortController()
      const fetch = vi.fn((_input: RequestInfo | URL, init?: RequestInit) =>
        rejectWhenAborted(signalOf(init))
      )
      vi.spyOn(AbortSignal, 'timeout').mockReturnValue(controller.signal)
      vi.stubGlobal('fetch', fetch)

      const title = fetchPageTitle('https://example.com')

      await vi.waitFor(() => expect(fetch).toHaveBeenCalledTimes(2))
      controller.abort()

      await expect(title).resolves.toBe(unavailableTitle)
    })

    test('returns null when a page fetch aborts at the timeout', async () => {
      const controller = new AbortController()
      const fetch = vi.fn((_input: RequestInfo | URL, init?: RequestInit) =>
        rejectWhenAborted(signalOf(init))
      )
      ...
      const title = fetchTitle('https://example.com')
  ...
  describe('redirects', () => {
    ...
    test('stops after three redirects', async () => {
      const fetch = vi
        .fn()
        .mockResolvedValueOnce(new Response('', { status: 302, headers: { location: '/one' } }))
        .mockResolvedValueOnce(new Response('', { status: 302, headers: { location: '/two' } }))
        .mockResolvedValueOnce(new Response('', { status: 302, headers: { location: '/three' } }))
        .mockResolvedValueOnce(new Response('', { status: 302, headers: { location: '/four' } }))
      vi.stubGlobal('fetch', fetch)

      await expect(fetchTitle('https://example.com')).resolves.toBe(unavailableTitle)
