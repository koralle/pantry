...
export type BookmarkEditorSubmitResult =
  ...
  | {
      readonly ok: false;
      readonly failureCode: UpdateBookmarkFailureCode | null;
    };

export interface BookmarkEditorProps {
  readonly initialData: BookmarkEditorData;
  readonly onUpdateBookmark: (
...
export const BookmarkEditor = ({
  ...
  // 明確に分かれる。
  const [editorError, setEditorError] = useState<BookmarkEditorError | null>(
    null
  );

  const clearFormFieldError = (field: BookmarkFormFieldKey) => {
    ...
    setEditorError((current) => {
      ...
      const nextForm: BookmarkEditorError["form"] = {
        ...(current.form.summary === undefined
          ? {}
          : { summary: current.form.summary }),
        ...(nextFields === undefined ? {} : { fields: nextFields }),
      };
      const isFormEmpty =
        nextForm.summary === undefined && nextForm.fields === undefined;
      return isFormEmpty ? {} : { form: nextForm };
    });
  };

  const handleSubmit = async (values: BookmarkFormSubmitValues) => {
    setEditorError(null);
    ...
    const updateResult = await onUpdateBookmark(
      buildUpdateBookmarkCommand(initialData, values)
    );
