...
interface UseBookmarkTitleFetchOptions {
...
export const useBookmarkTitleFetch = ({
  getUrl,
  setTitle,
  fetchTitleAction,
  onClearFieldError,
  ...
  const urlEmptyError =
    urlEmptyErrorShown && getUrl().trim() === "" ? urlRequiredMessage : null;

  const titleFetchError =
    urlEmptyError ??
    (titleFetchState.status === "error" && !isFetchingTitle
      ? titleFetchState.message
      : null);
  ...
  return { handleFetchTitle, isFetchingTitle, titleFetchError };
};
