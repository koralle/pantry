...
export const RejectsEmptyUrl = Default.extend({
  ...
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
    const fetchTitleAction =
      args.fetchTitleAction as Mock<BookmarkTitleFetchAction>;
    await userEvent.click(
      canvas.getByRole("button", { name: "タイトルを取得" })
    );
...
export const RetryClearsTitleFetchError = Default.extend({
  play: async ({ canvasElement, args }) => {
    ...
    await waitFor(
      () => {
        expect(canvas.getByLabelText("タイトル")).toHaveValue(
          "再び取得したタイトル"
        );
      },
      { timeout: 3000 }
...
export const TitleFetchPending = Default.extend({
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByRole("button", { name: "取得中…" })
    ).toBeDisabled();
...
export const SummaryError = Default.extend({
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getAllByText("別のURLを指定してください").length
...
function ControlledBookmarkForm({
  ...
  const [serverError, setServerError] =
    useState<BookmarkFormServerError | null>(initialServerError);
  ...
  function handleClearFieldError(field: BookmarkFormFieldKey) {
    setServerError((current) => {
      ...
      const nextFields =
        Object.keys(remainingFields).length === 0 ? undefined : remainingFields;
...
export const ServerFieldErrorShownInFieldAndSummary = Default.extend({
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByLabelText("URL")).toHaveAttribute(
      "aria-invalid",
      "true"
    );
  },
...
export const EditingClearsMatchingServerFieldError = Default.extend({
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getAllByText("タイトルを入力してください").length
    ).toBeGreaterThanOrEqual(1);
...
export const Pending = Default.extend({
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole("button", { name: "更新" }));
    await expect(
      canvas.getByRole("button", { name: "更新中…" })
    ).toBeDisabled();
    await expect(canvas.getByLabelText("URL")).toBeDisabled();
  },
});
...
export const SubmitsBrandedValues = Default.extend({
  ...
  play: async ({ canvasElement, args }) => {
    ...
    const onSubmit = args.onSubmit as Mock<
      (values: BookmarkFormSubmitValues) => void
    >;
    await userEvent.click(canvas.getByRole("button", { name: "更新" }));
...
export const CreatesAndSelectsNewTag = Default.extend({
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await userEvent.click(
      body.getByRole("button", { name: "「Python」を新しいタグとして作成" })
    );
    await waitFor(() => {
      expect(
...
export const InvalidTagErrorStaysOnTagField = Default.extend({
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getAllByText(/タグを選び直してください/).length
    ).toBeGreaterThanOrEqual(2);
    await expect(canvas.getByLabelText("URL")).toHaveValue(
      defaultInitialValues.url
    );
