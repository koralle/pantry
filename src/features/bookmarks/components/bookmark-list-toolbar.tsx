...
const displayTagName = (searchKey: string, shelfTags: ShelfTag[]): string =>
  shelfTags.find((tag) => tagNamesMatch(tag.name, searchKey))?.name ??
  searchKey;
      ...
      {selectedTags.length > 0 || addableTags.length > 0 ? (
        <div className={tagsRow}>
          ...
          {addableTags.length > 0 ? (
            ...
            >
              {addableTags.map((tag) => (
                <StyledSelect.Item key={tag.id} id={tag.name}>
                  {tag.name}
                </StyledSelect.Item>
              ))}
            </StyledSelect.Filterable>
