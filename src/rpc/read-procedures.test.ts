...
function baseDeps(): ReadDeps {
  return {
    ...
    insertTag: vi.fn(async (): Promise<InsertTagOutput> => ({
      id: 1 as never,
      kind: "created",
    })),
    ...
    updateTag: vi.fn(async (): Promise<UpdateTagOutput> => ({
      kind: "not-found",
    })),
  };
}
...
describe("auth.session RPC", () => {
  ...
  test("user の projection は id・name・email のみ", async () => {
    ...
    await expect(client.auth.session()).resolves.toStrictEqual({
      user: {
        email: "koralle@example.com",
        id: userId,
        name: "koralle",
...
describe("tags.shelf RPC", () => {
  ...
  test("認証済みは query service の棚行を返し、UserId を渡す", async () => {
    ...
    await expect(client.tags.shelf()).resolves.toStrictEqual(shelfTags);
    expect(listShelfTags).toHaveBeenCalledWith(userId);
  });

  test("wire 出力の id は number であり TagId ではない", () => {
    type ShelfOutput = Awaited<
      ReturnType<RouterClient<AppRouter>["tags"]["shelf"]>
    >;
...
describe("tags.list RPC", () => {
  test("pagination 入力を検証して query service へ渡す", async () => {
    ...
    await expect(
      client.tags.list({ limit: 20, offset: 40 })
    ).resolves.toStrictEqual([{ id: 1, name: "work" }]);
...
describe("tags.byId RPC", () => {
  ...
  test("見つかった場合は画面 projection だけを返す", async () => {
    ...
    await expect(client.tags.byId({ id: 7 })).resolves.toStrictEqual(record);
  ...
  test("wire 出力は projection 型であり brand や DB 行を載せない", () => {
    type ByIdOutput = Awaited<
      ReturnType<RouterClient<AppRouter>["tags"]["byId"]>
    >;
