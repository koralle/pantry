...
describe("CreateBookmark RPC", () => {
  test("create の戻り値 id は BookmarkId ではなく plain string である", () => {
    type CreateOutput = Awaited<
      ReturnType<RouterClient<AppRouter>["bookmarks"]["create"]>
    >;
  ...
  test("不正な入力は 400 BAD_REQUEST を返し、port を呼ばない", async () => {
    const insertBookmark = vi.fn(
      async () => ({ kind: "duplicate-url" }) as const
    );
    ...
    const rejected = await client.bookmarks
      .create({ ...validInput, url: "not-a-url" } as any)
      .then(
        () => null,
        (error: unknown) => error
      );
  ...
  test("想定外の例外は内部 message を漏らさず 500 を返す", async () => {
    ...
    await expect(client.bookmarks.create(validInput)).rejects.toBeInstanceOf(
      Error
    );
  ...
  test("成功は port が返した id を plain string で返す", async () => {
    ...
    const router = authenticatedRouter(async (input) => {
      ...
      return { id: id as never, kind: "created" };
    });
    const { client, getResponse } = createTestClient(router);

    await expect(client.bookmarks.create(validInput)).resolves.toStrictEqual({
      id,
    });
