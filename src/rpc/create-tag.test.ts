...
describe("CreateTag RPC", () => {
  test("create の戻り値 id は TagId ではなく number である", () => {
    type CreateOutput = Awaited<
      ReturnType<RouterClient<AppRouter>["tags"]["create"]>
    >;
  ...
  test("想定外の例外は 500 を返す", async () => {
    ...
    await expect(client.tags.create({ name: "Work" })).rejects.toBeInstanceOf(
      Error
    );
  ...
  test("handleRpcRequest は handler.handle() の Response を返す", async () => {
    ...
    await expect(client.tags.create({ name: "Work" })).resolves.toStrictEqual({
      id: 7,
    });
