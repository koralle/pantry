...
function preferredPort() {
  if (process.env.BRAINSTORM_PORT) {
    return Number(process.env.BRAINSTORM_PORT)
  }
  if (PORT_FILE) {
    try {
      const p = Number(fs.readFileSync(PORT_FILE, 'utf8').trim())
      if (Number.isInteger(p) && p > 1023 && p < 65_536) {
        return p
      }
    } catch {
...
function getNewestScreen() {
  const files = fs
    ...
    .map((f) => {
      ...
      if (!isRegularFileInsideContentDir(fp)) {
        return null
      }
...
function isAuthorized(req) {
  ...
  if (cookie && timingSafeEqualStr(cookie, TOKEN)) {
    return true
  }
  return false
}
  ...
  if (
    ...
    let html = screenFile
      ? ((raw) => (isFullDocument(raw) ? raw : wrapInFrame(raw)))(
          fs.readFileSync(screenFile, 'utf8')
        )
      : waitingPage()
...
function maybeOpenBrowser() {
  ...
  browserOpened = true
  if (!process.env.BRAINSTORM_OPEN) {
    return
  } // Opt-in: only after the user approves the companion
  if (HOST !== '127.0.0.1' && HOST !== 'localhost') {
    return
  }
  if (clients.size > 0) {
    return
  } // The user already opened it
