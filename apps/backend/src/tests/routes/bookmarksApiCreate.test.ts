...
describe("POST /v1/bookmarks", () => {
  test(
    "[TEST-INT-001] 有効な body を指定したとき、POST /v1/bookmarks を呼ぶと、body を service に渡して 201 を返す",
    async () => {
      const result = {
  ...
  test(
    "[TEST-INT-003] service が URL_CONFLICT を返すとき、POST /v1/bookmarks を呼ぶと、409 URL_CONFLICT を返す",
    async () => {
  ...
  test(
    "[TEST-INT-001] 不正な url を含む body を指定したとき、POST /v1/bookmarks を呼ぶと、400 INVALID_INPUT を返して service を呼ばない",
    async () => {
      const createMock = vi
        .fn<BookmarksService["create"]>()
  ...
  test(
    "[TEST-INT-001] tags を 21 件含む body を指定したとき、POST /v1/bookmarks を呼ぶと、400 INVALID_INPUT を返して service を呼ばない",
    async () => {
      const createMock = vi
        .fn<BookmarksService["create"]>()
        .mockResolvedValue({ bookmark: createMockBookmarkDetail() });
  ...
  test(
    "[TEST-INT-001] 33 文字以上の tag を含む body を指定したとき、POST /v1/bookmarks を呼ぶと、400 INVALID_INPUT を返して service を呼ばない",
    async () => {
      const createMock = vi
        .fn<BookmarksService["create"]>()
        .mockResolvedValue({ bookmark: createMockBookmarkDetail() });
  ...
  test(
    "[TEST-INT-001] 空文字の tag を含む body を指定したとき、POST /v1/bookmarks を呼ぶと、400 INVALID_INPUT を返して service を呼ばない",
    async () => {
      const createMock = vi
        .fn<BookmarksService["create"]>()
        .mockResolvedValue({ bookmark: createMockBookmarkDetail() });
