...
describe("GET /v1/bookmarks/:bookmarkId", () => {
  test(
    "[TEST-INT-001] 有効な bookmarkId を指定したとき、GET /v1/bookmarks/:bookmarkId を呼ぶと、service の結果を返して 200 を返す",
    async () => {
      const result = {
  ...
  test(
    "[TEST-INT-001] 不正な bookmarkId を指定したとき、GET /v1/bookmarks/:bookmarkId を呼ぶと、400 INVALID_INPUT を返して service を呼ばない",
    async () => {
      const getMock = vi
  ...
  test(
    "[TEST-INT-001] service が NOT_FOUND を返すとき、GET /v1/bookmarks/:bookmarkId を呼ぶと、404 NOT_FOUND を返す",
    async () => {
