...
describe("DELETE /v1/bookmarks/:bookmarkId", () => {
  test("[TEST-INT-023] 有効な bookmarkId を指定したとき、DELETE /v1/bookmarks/:bookmarkId を呼ぶと、service を呼び出して 204 を返す", async () => {
    const deleteMock = vi.fn<BookmarksService["delete"]>().mockResolvedValue(undefined);
  ...
  test("[TEST-INT-023] 不正な bookmarkId を指定したとき、DELETE /v1/bookmarks/:bookmarkId を呼ぶと、400 INVALID_INPUT を返して service を呼ばない", async () => {
    const deleteMock = vi.fn<BookmarksService["delete"]>().mockResolvedValue(undefined);
    const app = createApp(createMockDependencies({ delete: deleteMock }));
  ...
  test("[TEST-INT-023] service が NOT_FOUND を返すとき、DELETE /v1/bookmarks/:bookmarkId を呼ぶと、404 NOT_FOUND を返す", async () => {
