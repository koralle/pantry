...
describe("PATCH /v1/bookmarks/:bookmarkId", () => {
  test("[TEST-INT-004] note: null を含む body を指定したとき、PATCH /v1/bookmarks/:bookmarkId を呼ぶと、note: null を service に渡して 200 を返す", async () => {
    const result = {
      bookmark: createMockBookmarkDetail(),
    };
    const updateMock = vi.fn<BookmarksService["update"]>().mockResolvedValue(result);
  ...
  test("[TEST-INT-004] 不正な body を指定したとき、PATCH /v1/bookmarks/:bookmarkId を呼ぶと、400 INVALID_INPUT を返して service を呼ばない", async () => {
    const updateMock = vi
  ...
  test("[TEST-INT-003] service が URL_CONFLICT を返すとき、PATCH /v1/bookmarks/:bookmarkId を呼ぶと、409 URL_CONFLICT を返す", async () => {
